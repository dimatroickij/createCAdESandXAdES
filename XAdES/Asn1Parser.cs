...
namespace SignatureMaker.WinForm.Xades
{
	...
	{
		private const int TagNumberMask = 0x1F; //Bits 5 - 1
		private const int TagConstructedFlagMask = 0x20; //Bit 6
		private const int TagClassMask = 0xC0; //Bits 7 - 8
		private const int LenXtnd = 0x80; //Indefinite or long form
		private const long LenMask = 0x7F; //Bits 7 - 1
		private const int MaxOidSize = 32;
		...
		public XmlNode ParseTree { get; private set; }
		...
		private string CheckText(byte[] byteArrayToTest)
		{
			...
			{
				var charToTest = (char) byteArrayToTest[charCounter];
				itIsText &= (char.IsLetterOrDigit(charToTest) |
				             char.IsWhiteSpace(charToTest) |
				             char.IsPunctuation(charToTest));
		...
		private Asn1ItemData GetItemData(byte[] currentBytesToParse, bool throwErrors, ref long index)
		{
			...
			try
			{
				...
				if ((retVal.BytesToParseLength & LenXtnd) != 0)
				{
					//We have a multiple byte length
					lengthLength = retVal.BytesToParseLength & LenMask; //Strip bit
					if (lengthLength > 4)
					{
		...
		private void ParseAsn1Item(XmlNode currentXmlNode, byte[] currentBytesToParse, long index)
		{
			...
			{
				var asn1ItemData = GetItemData(currentBytesToParse, true, ref index);
				...
				if (asn1ItemData.TagConstructedFlag == TagConstructed.Constructed)
				{
					...
					ParseAsn1Item(newChildXmlNode, asn1ItemData.BytesToParse, indexForNextRecursion);
				}
				else
				{
					valueBuffer = "";
					if (asn1ItemData.TagClass == TagClasses.Universal)
					{
						switch (asn1ItemData.TagNumber)
						{
							case UniversalTags.Boolean:
								...
								{
									throw new Exception("BOOLEAN containf wrong value (" +
									                    asn1ItemData.BytesToParse[0].ToString("X2") + ")");
								}
								else
								{
							...
							case UniversalTags.Oid:
								if (asn1ItemData.BytesToParseLength > MaxOidSize)
								{
									throw new Exception("OID length (" + asn1ItemData.BytesToParseLength +
									                    ") > MAX_OID_SIZE");
								}
								...
								buffer = 0;
								for (lengthCounter = 1;
									lengthCounter < asn1ItemData.BytesToParseLength;
									lengthCounter++)
								{
							...
							case UniversalTags.OctetString:
								...
								var probeAsn1ItemData = GetItemData(asn1ItemData.BytesToParse, false,
									ref indexForNextRecursion);
								if (probeAsn1ItemData != null)
								{
									if (probeAsn1ItemData.BytesToParseLength + probeAsn1ItemData.HeaderLength ==
									    asn1ItemData.BytesToParseLength)
									{
										indexForNextRecursion = 0;
										ParseAsn1Item(newChildXmlNode, asn1ItemData.BytesToParse,
											indexForNextRecursion);
									}
									else
									{
										for (lengthCounter = 0;
										...
										{
											valueBuffer +=
												asn1ItemData.BytesToParse[lengthCounter].ToString("X2") + " ";
										}
							...
							case UniversalTags.GeneralizedTime:
								...
								{
									throw new Exception("Genralized time has to be at least 15 bytes long (" +
									                    asn1ItemData.BytesToParseLength.ToString() + ")");
								}
								else
								{
									valueBuffer = GetTime(asn1ItemData.BytesToParse, false);
									newXmlAttribute = currentXmlNode.OwnerDocument.CreateAttribute("Value");
									newXmlAttribute.Value = valueBuffer;
									newChildXmlNode.Attributes.Append(newXmlAttribute);
								}
							...
							case UniversalTags.UtcTime:
								...
								{
									throw new Exception("UTC time has to be 13 bytes long (" +
									                    asn1ItemData.BytesToParseLength.ToString() + ")");
								}
								else
								{
									valueBuffer = GetTime(asn1ItemData.BytesToParse, true);
									newXmlAttribute = currentXmlNode.OwnerDocument.CreateAttribute("Value");
									newXmlAttribute.Value = valueBuffer;
									newChildXmlNode.Attributes.Append(newXmlAttribute);
								}
							...
							case UniversalTags.BitString:
								...
								{
									throw new Exception("Unused bits of bistring out of range [1-7] (" +
									                    unusedBits.ToString() + ")");
								}
								...
								if (asn1ItemData.BytesToParseLength - 1 < 5)
								{
										...
										lengthCounter++)
									{
										if (lengthCounter != (asn1ItemData.BytesToParseLength - 1))
										{
											...
											{
												if (((bitmask >> bitCounter) &
												     asn1ItemData.BytesToParse[lengthCounter]) == 0)
												{
										...
										{
											for (var bitCounter = 0; bitCounter < (8 - unusedBits); bitCounter++)
											{
												if (((bitmask >> bitCounter) &
												     asn1ItemData.BytesToParse[lengthCounter]) == 0)
												{
					...
					{
						valueBuffer = CheckText(asn1ItemData.BytesToParse);
						if (valueBuffer == "")
						{
		...
		public void ParseAsn1(byte[] bytesToParse)
		{
			...
			ParseTree = newXmlDocument.CreateElement("Asn1Root");
			ParseAsn1Item(ParseTree, bytesToParse, bytesToParseIndex);
		}
	}
	...
	public class Asn1ItemData
	{
		...
		public TagConstructed TagConstructedFlag { get; set; }
		public UniversalTags TagNumber { get; set; }
		public string TagName { get; set; }
		public long HeaderLength { get; set; }
		public long BytesToParseLength { get; set; }
		public byte[] BytesToParse { get; set; }
	}
